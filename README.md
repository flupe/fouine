...
- Run with `./fouine file.ml` to run a given file before starting the REPL.

- Use option `-machine` to instead compile the parsed expressions to a custom SECD bytecode using the `Compiler` module, and then run that bytecode using the `Secd` module.

- Use option `-autotest` to compare the output of the interpreter with that of the machine.
...
We also hit a few roadblocks, in order to isolate the calls to `ref`, to `:=` or to `!` in the AST (because they now appear just like any other function calls), and to change the signature of the builtin operators from `a -> b` to `a -> state -> (b * state)` because of the way we transform function calls.
